...
public class LimitService {
    ...
    public LimitResponseDto updateLimits(LimitRequestDto updateLimitRequestDto) {
        ...
        List<Limit> updateLimits = limits.stream()
                                         .map(limit -> limitMapper.map(limit, updateLimitRequestDto))
                                         .toList();
        limitRepository.saveAll(updateLimits);
    ...
    public void updateLimitsCron() {
        List<Limit> limits = limitRepository.findAll();
        List<Limit> updateLimits = limits.stream()
                                         .peek(limit -> limit.setAmountLimit(defaultAmountLimit))
                                         .toList();
        limitRepository.saveAll(updateLimits);
